...
// -----------------------------------------------------------------------------
static inline bool addr_valid(const uint16_t addr) { return EEPROM_ADDR_MAX >= addr; }
...
static bool eeprom_read_byte(const uint16_t addr, uint8_t* byte)
{
    ...
    if (!addr_valid(addr) || (NULL == byte)) { return false; }
...
// -----------------------------------------------------------------------------
int16_t eeprom_write(const uint8_t* buf, const uint16_t buflen, const uint16_t addr)
{
    // Check the input arguments, return -1 if invalid.
    if ((NULL == buf) || (0U == buflen) || !addr_valid(addr)) { return -1; }
    uint16_t i = 0U;
    ...
    for (; i < buflen; ++i)
    {
        ...
        if (!eeprom_write_byte(addr + i, buf[i])) { break; }
    }
...
int16_t eeprom_read(uint8_t* buf, const uint16_t buflen, const uint16_t addr)
{
    ...
    for (; i < buflen; ++i)
    {
        ...
        if (!eeprom_read_byte(addr + i, buf + i)) { break; }
    }
