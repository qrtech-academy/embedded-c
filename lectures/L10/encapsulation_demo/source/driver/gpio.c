...
typedef struct gpio {
  ...
  /** Pin change interrupt control register bit (PCIEx) associated with the
   * GPIO's port. */
  uint8_t pcicrx;
...
// -----------------------------------------------------------------------------
gpio_t *gpio_new(const uint8_t pin, const gpio_mode_t mode,
                 gpio_callback_t cb) {
...
void gpio_toggle(gpio_t *self) {
  ...
  // Toggle the output by setting the pin in PINx (the hardware will toggle the
  // output).
...
bool gpio_handle_event(gpio_t *self) {
  ...
  if (gpio_event_occurred(self)) {
    // Invoke callback if an event has occurred, mark as handled and clear the
    // event flag.
    if (NULL != self->cb) {
      self->cb();
      handled = true;
